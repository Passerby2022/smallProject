$(function(){
    ...
    function getData(){
        $.ajax({
            ...
            success:function(res){
                ...
                //渲染成功
                if(res.status===0){
                    let contentStr = template("addDepartment",res);
                    //通过模板引擎将内容渲染到页面中
    ...
    function addData(BnoData,BnData){
        $.ajax({
            method:'POST',
            url:'http://192.168.253.1:3000/api/addDepartment',
            data:{
                Bno:BnoData,
                Bn:BnData
            },
    ...
    //监听添加按钮的点击事件
    $("#addData").on('click',debounce(function(){
        //前端简单检测部门编号和部门名称是否符合规则
        if((Bno.length===0)&&(Bn.length===0)){
            alert('部门编号和部门名称不能为空');
            //提交完数据后表单全部清空
            clearText();
        }
