$(function(){
    ...
    function getData(){
        $.ajax({
            ...
            success:function(res){
                ...
                //渲染成功
                if(res.status===0){
                    let contentStr = template("addPayRTS",res);
                    //通过模板引擎将内容渲染到页面中
    ...
    function addPayRTSData(ZnoData,PayData,BonusData,RTSData){
        $.ajax({
            method:'POST',
            url:'http://192.168.253.1:3000/api/addPayRTS',
            data:{
                Zno:ZnoData,
                Pay:PayData,
                bonus:BonusData,
    ...
    //监听添加按钮的点击事件
    $("#addPayRTSData").on('click',debounce(function(){
        ...
        addPayRTSData(Zno,Pay,bonus,rts);
        //提交完数据后表单全部清空
        clearText();
    },1000))
