$(function(){
    ...
    function selectData(BnData){
        $.ajax({
            method:'POST',
            url:'http://192.168.253.1:3000/api/selectDepartment',
            data:{
                Bn:BnData
            },
            success:function(res){
                ...
                //查询成功
                if(res.status===0){
                    let contentStr = template("selectDepartment",res);
                    //通过模板引擎将内容渲染到页面中
                    $('#indexBody').html(contentStr);
                    let contentHeadStr = template("selectDepartmentHead");
    ...
    //监听添加按钮的点击事件
    $("#selectData").on('click',debounce(function(){
        //前端简单检测部门名称是否符合规则
        if(Bn.length===0){
            alert('部门名称不能为空');
            //提交完数据后表单全部清空
