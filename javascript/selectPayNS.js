$(function(){
    ...
    function selectPayNSData(ZnoData){
        $.ajax({
            method:'POST',
            url:'http://192.168.253.1:3000/api/selectPayNS',
            data:{
                Zno:ZnoData
            },
            success:function(res){
                ...
                //后端验证通过渲染页面
                if(res.status===0){
                    let contentStr = template("selectPayNS",res);
                    //通过模板引擎将内容渲染到页面中
                    $('#indexBody').html(contentStr);
                    let contentHeadStr = template("selectPayNSHead");
    ...
    //监听添加按钮的点击事件
    $("#selectPayNSData").on('click',debounce(function(){
