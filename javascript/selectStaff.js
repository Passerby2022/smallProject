$(function(){
    ...
    function selectStaffData(ZnoData){
        $.ajax({
            method:'POST',
            url:'http://192.168.253.1:3000/api/selectStaff',
            data:{
                Zno:ZnoData
            },
            success:function(res){
                ...
                //查询成功
                if(res.status===0){
                    let contentStr = template("selectStaff",res);
                    //通过模板引擎将内容渲染到页面中
                    $('#indexBody').html(contentStr);
                    let contentHeadStr = template("selectStaffHead");
    ...
    //监听添加按钮的点击事件
    $("#selectStaffData").on('click',debounce(function(){
        //前端简单检测职员编号是否符合规则
        if(Zno.length===0){
            alert('职员编号不能为空');
            //提交完数据后表单全部清空
