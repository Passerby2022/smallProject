$(function(){
    ...
    function selectPayRTSData(ZnoData){
        $.ajax({
            method:'POST',
            url:'http://192.168.253.1:3000/api/selectPayRTS',
            data:{
                Zno:ZnoData
            },
            success:function(res){
                ...
                //后端验证通过渲染页面
                if(res.status===0){
                    let contentStr = template("selectPayRTS",res);
                    //通过模板引擎将内容渲染到页面中
                    $('#indexBody').html(contentStr);
                    let contentHeadStr = template("selectPayRTSHead");
    ...
    //监听添加按钮的点击事件
    $("#selectPayRTSData").on('click',debounce(function(){
