$(function(){
    ...
    $('#modifyBno').on('keyup',debounce(function(){
        //trim()是清除字符串前后的空格符
        modifyBno = $('#modifyBno').val().trim();
    },100));

    $('#modifyBn').on('keyup',debounce(function(){
        //trim()是清除字符串前后的空格符
        modifyBn = $('#modifyBn').val().trim();
    },100));
    ...
    function getData(){
        $.ajax({
            ...
            success:function(res){
                ...
                //渲染成功
                if(res.status===0){
                    let contentStr = template("modifyDepartment",res);
                    //通过模板引擎将内容渲染到页面中
    ...
    function modifyData(BnData,modifyBnoData,modifyBnData){
        $.ajax({
            method:'POST',
            url:'http://192.168.253.1:3000/api/modifyDepartment',
            data:{
                Bn:BnData,
                modifyBno:modifyBnoData,
                modifyBn:modifyBnData
            },
            success:function(res){
                //后端验证不通过
                if(res.status===1){
    ...
    $("#modifyData").on('click',debounce(function(){
        ...
        modifyData(Bn,modifyBno,modifyBn);
        //提交完数据后表单全部清空
        clearText();
    },1000))
