$(function(){
    ...
    function getData(){
        $.ajax({
            ...
            success:function(res){
                ...
                //渲染成功
                if(res.status===0){
                    let contentStr = template("modifyStaff",res);
                    //通过模板引擎将内容渲染到页面中
    ...
    function modifyStaffData(ZnoData,ZnData,SexData,AgeData,PhoneData,EducationData,BnoData,DutiesData,TitleData){
        $.ajax({
            method:'POST',
            url:'http://192.168.253.1:3000/api/modifyStaff',
            data:{
                Zno:ZnoData,
                Zn:ZnData,
                Sex:SexData,
    ...
    //监听添加按钮的点击事件
    $("#modifyData").on('click',debounce(function(){
        //前端简单检测是否符合规则
        let ZnoFlag = inspectText(Zn.length,'职员编号');
        ...
        if(ZnoFlag&&ZnFlag&&SexFlag&&AgeFlag&&PhoneFlag&&EducationFlag&&BnoFlag&&DutiesFlag&&TitleFlag){
            modifyStaffData(Zno,Zn,Sex,Age,Phone,Education,Bno,Duties,Title);
            //提交完数据后表单全部清空
            clearText();
        }
    },1000))
