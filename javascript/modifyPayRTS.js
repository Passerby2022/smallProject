$(function(){
    ...
    $('#Pay').on('keyup',debounce(function(){
        //trim()是清除字符串前后的空格符
        Pay = $('#Pay').val().trim();
    },100))

    $('#Bonus').on('keyup',debounce(function(){
        //trim()是清除字符串前后的空格符
    ...
    function getData(){
        $.ajax({
            ...
            success:function(res){
                ...
                //渲染成功
                if(res.status===0){
                    let contentStr = template("modifyPayRTS",res);
                    //通过模板引擎将内容渲染到页面中
    ...
    function modifyPayRTSDataData(ZnoData,PayData,BonusData,RTSData){
        $.ajax({
            method:'POST',
            url:'http://192.168.253.1:3000/api/modifyPayRTS',
            data:{
                Zno:ZnoData,
                Pay:PayData,
                bonus:BonusData,
    ...
    //监听添加按钮的点击事件
    $("#modifyPayRTSData").on('click',debounce(function(){
        //前端简单检测部门编号和部门名称是否符合规则
        if((Zno.length===0)&&(Pay.length===0)&&(bonus.length===0)){
            alert('职员编号，基本工资，浮动奖金都不能为空');
            //提交完数据后表单全部清空
            clearText();
        }
        rts = Number(Number(Pay)+Number(bonus)).toFixed(2);
        modifyPayRTSDataData(Zno,Pay,bonus,rts);
        //提交完数据后表单全部清空
        clearText();
    },1000))
