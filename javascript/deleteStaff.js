$(function(){
    ...
    function getData(){
        $.ajax({
            ...
            success:function(res){
                ...
                //渲染成功
                if(res.status===0){
                    let contentStr = template("deleteStaff",res);
                    //通过模板引擎将内容渲染到页面中
    ...
    function deleteData(ZnoData){
        $.ajax({
            method:'POST',
            url:'http://192.168.253.1:3000/api/deleteStaff',
            data:{
                Zno:ZnoData
            },
            success:function(res){
    ...
    //监听添加按钮的点击事件
    $("#deleteStaffData").on('click',debounce(function(){
        //前端简单检测部门名称是否符合规则
        if(Zno.length===0){
            alert('部门名称不能为空');
            //提交完数据后表单全部清空
