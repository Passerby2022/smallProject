$(function(){
    ...
    boxFlag++;
    $('.two').siblings('ul').toggleClass('showhide');
    twoFlag++;
    ...
    $('#DT').on('keyup',debounce(function(){
        //trim()是清除字符串前后的空格符
        dt = $('#DT').val().trim();
    },100));

    $('#IIT').on('keyup',debounce(function(){
        //trim()是清除字符串前后的空格符
        iit = $('#IIT').val().trim();
    },100))
    ...
    function getData(){
        $.ajax({
            method:'GET',
            url:'http://192.168.253.1:3000/api/rederingPayNS',
            success:function(res){
                ...
                //渲染成功
                if(res.status===0){
                    let contentStr = template("modifyPayNS",res);
                    //通过模板引擎将内容渲染到页面中
    ...
    function modifyPayNSData(ZnoData,DTData,IITData,NSData){
        $.ajax({
            method:'POST',
            url:'http://192.168.253.1:3000/api/modifyPayNS',
            data:{
                Zno:ZnoData,
                dt:DTData,
                iit:IITData,
    ...
    function getPayData(Znodata){
        $.ajax({
            method:'POST',
            url:'http://192.168.253.1:3000/api/rederingss',
            data:{
                Zno:Znodata
            },
            success:function(res){
                ...
                if(res.status===0){
                    ss = res.ss;
                    ns = Number(Number(iit)+ss).toFixed(2);
                    modifyPayNSData(Zno,dt,iit,ns);
                    //提交完数据后表单全部清空
                    clearText();
                }
            }
    ...
    //监听添加按钮的点击事件
    $("#modifyPayNSData").on('click',debounce(function(){
        //前端简单检测部门编号和部门名称是否符合规则
        if((Zno.length===0)&&(dt.length===0)&&(iit.length===0)){
