$(function(){
    ...
    boxFlag++;
    $('.three').siblings('ul').toggleClass('showhide');
    threeFlag++;
    ...
    $('#Subsidy').on('keyup',debounce(function(){
        //trim()是清除字符串前后的空格符
        Subsidy = $('#Subsidy').val().trim();
    },100))

    $('#Deduction').on('keyup',debounce(function(){
        //trim()是清除字符串前后的空格符
        Deduction = $('#Deduction').val().trim();
    },100));

    $('#Insure').on('keyup',debounce(function(){
        //trim()是清除字符串前后的空格符
        Insure = $('#Insure').val().trim();
    },100));

    function getData(){
        $.ajax({
            method:'GET',
            url:'http://192.168.253.1:3000/api/rederingPaySS',
            success:function(res){
                ...
                //渲染成功
                if(res.status===0){
                    let contentStr = template("modifyPaySS",res);
                    //通过模板引擎将内容渲染到页面中
    ...
    function modifyPaySSData(ZnoData,SubsidyData,DeductionData,InsureData,SSData){
        $.ajax({
            method:'POST',
            url:'http://192.168.253.1:3000/api/modifyPaySS',
            data:{
                Zno:ZnoData,
                Subsidy:SubsidyData,
    ...
    function getPayData(Znodata){
        $.ajax({
            method:'POST',
            url:'http://192.168.253.1:3000/api/redering',
            data:{
                Zno:Znodata
            },
            success:function(res){
                ...
                if(res.status===0){
                    rts = res.rts;
                    ss = Number(Number(Subsidy)+Number(Deduction)+Number(Insure)+rts).toFixed(2);
                    console.log(rts);
                    modifyPaySSData(Zno,Subsidy,Deduction,Insure,ss);
                    //提交完数据后表单全部清空
                    clearText();
                }
            }
    ...
    //监听添加按钮的点击事件
    $("#modifyPaySSData").on('click',debounce(function(){
        //前端简单检测部门编号和部门名称是否符合规则
        if((Zno.length===0)&&(Subsidy.length===0)&&(Deduction.length===0)&&(Insure.length===0)){
